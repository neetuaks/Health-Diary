...
## OCR status
- **Camera/OCR entry is being re-tested on `feature/ocr-seven-segment-decoder` (not on `main`).** ML Kit/Vision reads printed text (SYS/DIA/PULSE labels, mg/dL, mmol) fine but misreads seven-segment LCD digits itself, so `src/services/ocr.ts` now also runs a pure-JS pixel decoder, `src/services/sevenSegmentDecoder.ts` (deskew -> Sauvola threshold -> connected components -> structural digit grouping -> segment classification), and uses the native module's `rawText` only to classify bp-vs-glucose; it falls back to the original `rawText`-only parse whenever the decoder's row count/values don't fit. The `onScanPhoto` prop passed to `NewRecordModal` in `src/screens/DiaryScreen.tsx` is restored on this branch.
- **Known accuracy limit — read before re-enabling on `main`:** the decoder is tuned to the two device fonts in `docs/ocr-samples/` (Omron HEM-7111, Accu-Chek Active) and gets those 4/4. On a broader crowdsourced sample of other brands it only got 3/12 right (see `docs/PHOTO-GUIDE.md`), so most other devices will still auto-fill wrong values — the "double-check before saving" banner is load-bearing, not decorative, until the decoder (or a learned classifier) generalizes further.
- All OCR text interpretation lives in one JS parser, `src/services/ocrParsing.ts` (labeled SYS/DIA/PULSE, slash `120/80`, unlabeled stacked numbers, with plausibility checks — `validBP`/`validPulse` are exported and reused by the pixel-decoder fusion in `ocr.ts`; unit-tested). The native modules (`native/android/MLKitOCR.kt`, `native/ios/VisionOCR.swift`) are trusted only for the recognized `rawText`.
...
- The pixel decoder is pure JS/TS (`jpeg-js` for decoding, no native code), so testing it on-device needs no new EAS build — `npx expo start --dev-client` and reload the existing dev client picks up the new JS bundle.
