        ...
        tx.executeSql('SELECT * FROM parameter_types;', [], async (_, ptRes) => {
          ...
          tx.executeSql('SELECT * FROM readings;', [], async (_, rRes) => {
            ...
            profiles.forEach((p: any) => {
              db.transaction(tx => {
                tx.executeSql('UPDATE profiles SET last_backup_at = ? WHERE id = ?;', [now, p.id]);
              });
            });
...
export async function restoreEncryptedBackupFromFile(containerJson: string, recoveryKey?: string) {
  ...
  await new Promise<void>((resolve, reject) => {
    db.transaction(tx => {
      ...
      (obj.profiles || []).forEach((p: any) => {
        try {
          tx.executeSql('INSERT OR REPLACE INTO profiles (id, name, date_of_birth, glucose_unit_pref, weight_unit_pref, last_backup_at) VALUES (?,?,?,?,?,?);', [p.id, p.name, p.date_of_birth || null, p.glucose_unit_pref || 'mg/dL', p.weight_unit_pref || 'kg', p.last_backup_at || null]);
        } catch (e) { }
      });
      (obj.readings || []).forEach((r: any) => {
        try {
          tx.executeSql('INSERT OR REPLACE INTO readings (id, profile_id, parameter_type_id, recorded_at, created_at, source, values, notes) VALUES (?,?,?,?,?,?,?,?);', [r.id, r.profile_id, r.parameter_type_id, r.recorded_at, r.created_at || new Date().toISOString(), r.source || 'manual', JSON.stringify(r.values || {}), r.notes || null]);
        } catch (e) { }
      });
